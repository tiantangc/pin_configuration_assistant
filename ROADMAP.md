...
| Git | 建仓库推送到 GitHub | ⏳ 待用户本地执行（终端受限） |
...
### M2：锁定引脚 + CubeMX 集成（建议下一步）

- [ ] **锁定引脚重新求解**：某外设（或某角色）手动指定引脚后，剩余部分自动重新分配
- [ ] **生成 CubeMX 配置步骤**：按方案输出"GPIO 模式 / AF 复用 / 重映射 / 定时器通道"配置清单，照着手动配 CubeMX
- [ ] **导出引脚表**：Markdown / CSV 格式，发给队友
