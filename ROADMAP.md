...
| M1 | 网页版（NiceGUI）：动态增减外设行、保留引脚行、I2C 共享组、无方案原因诊断 | ✅ 完成 |
| Git | 建仓库推送到 GitHub | ✅ 已推送 https://github.com/tiantangc/pin_configuration_assistant |
| M2 | 锁定引脚重算 / CubeMX 步骤生成 / 导出引脚表 / 引脚图可视化 | ✅ 完成 |
| M3 | MSPM0G3507 天猛星开发板支持 | ✅ 第一版完成，待测试验证 |
| M4 | 方案保存 / 软件模拟建议 | ✅ 完成 |
| M5 | 场景模板（长期大饼，电赛明年用） | ⏳ 暂缓 |
| M6 | F407 支持（最后做） | 🔲 待做 |
...
- [x] **引脚图可视化**：芯片图（F103）+ 板子排针图（F103 两排 / MSPM0 四列），着色+引线标注+大图+下载

### M3：MSPM0G3507 天猛星板（第一版完成，待测试）

- [x] 用户整理天猛星板原理图/引脚引出表
- [x] 手工录入 `chips/MSPM0G3507.json`（PINCM 灵活复用建模）
- [x] 求解器兼容灵活复用格式（UART/I2C/SPI/TIMER/ADC）
- [x] 芯片切换、天猛星板预览/引脚图
- [x] 生成 TI SysConfig 配置步骤
- [ ] 用户完整测试验证

### M4：方案保存与软件模拟建议（已完成）

- [x] 方案保存/命名/对比（网页内历史记录，保存到 saved_solutions/ 目录）
- [x] 软件模拟建议：硬件不够时自动提示"这路可软件 I2C / 软件 PWM / GPIO 模拟编码器"
